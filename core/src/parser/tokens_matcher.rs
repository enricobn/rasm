...
impl TokensMatcher {
    pub fn new(name: &str, quantifier: Quantifier) -> Self {
        Self {
            group: TokensGroup::new(vec![name.to_string()], quantifier),
        }
    }
...
impl TokensMatcherResult {
    ...
    pub fn group_results(&self, name: &str) -> Vec<&Self> {
        let mut result: Vec<&Self> = if let Some(results) = self.groups_results.get(name) {
...
mod tests {
    ...
    fn new_result_with_inner_results(
        name: &str,
        group_values: Vec<TokensMatcherResult>,
    ) -> TokensMatcherResult {
    ...
    fn test_simple() {
        ...
        if let Some(match_result) = matcher.match_tokens(&test_namespace(), &parser, 0) {
    ...
    fn not_matching_test() {
        ...
        let match_result = matcher.match_tokens(&test_namespace(), &parser, 0);
    ...
    fn test_param_types_group() {
        ...
        if let Some(match_result) = param_types.match_tokens(&test_namespace(), &parser, 0) {
