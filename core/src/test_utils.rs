...
pub fn project_to_ast_typed_module_with_macros(
    project: &RasmProject,
    target: &CompileTarget,
    profile: &RasmProfile,
) -> Result<(ASTTypedModule, Statics), Vec<CompilationError>> {
    ...
    let (mut container, catalog, parse_errors) = project.container_and_catalog(&profile, &target);
    ...
    loop {
        ...
        let command_line_options = CommandLineOptions::new(CommandLineAction::Build);
        let out_folder = TempDir::new("rasm_macro_test").unwrap().into_path();
        resolve_macros(
            project,
            target,
            &extractor,
