    ...
    for enum_def in module.enums.iter() {
        ...
        for (variant_num, variant) in enum_def.variants.iter().enumerate() {
            ...
            let body_str = if variant.parameters.is_empty() {
                ...
                CodeGen::add(&mut native_body, &format!("push    {} {}", backend.word_size(), backend.word_len()), None, true);
                CodeGen::add(&mut native_body, "call    malloc", None, true);
                CodeGen::add(&mut native_body, &format!("add   {}, {}", backend.stack_pointer(), backend.word_len()), None, true);

                CodeGen::add(&mut native_body, &format!("mov   {} [{label}], eax", backend.word_size()), None, true);
                CodeGen::add(&mut native_body, &format!("mov   {} eax, [eax]", backend.word_size()), None, true);
                CodeGen::add(&mut native_body, &format!("mov   {} [eax], {variant_num}", backend.word_size()), None, true);
            ...
            functions_by_name.insert(enum_def.name.clone() + "::" + &variant.name.clone(), function_def);
        }
