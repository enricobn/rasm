...
impl TypeConversionContext {
    ...
    pub fn try_add_new(&mut self, original_name: &String, function_def: &ASTFunctionDef) -> Option<ASTFunctionDef> {
        ...
        if let Some(same_name_functions) = self.new_function_defs.get_mut(original_name) {
            if let Some(already_present) = same_name_functions.iter().find(|it| it.parameters == function_def.parameters && it.return_type == function_def.return_type) {
            ...
            self.new_function_defs.insert(original_name.clone(), same_name_functions);
            None
        }
    }

    pub fn replace_body(&mut self, function_def: &ASTFunctionDef) {
        if self.used_untyped_function_defs.contains_key(&function_def.name.clone()) {
            self.used_untyped_function_defs.insert(function_def.name.clone(), function_def.clone());
        } else {
    ...
    pub fn add_untyped(&mut self, function_def: &ASTFunctionDef) -> bool {
        if self.used_untyped_function_defs.contains_key(&function_def.name) {
    ...
    pub fn get(&self, name: &str) -> Option<&ASTFunctionDef> {
        if let Some(function_def) = self.used_untyped_function_defs.get(name) {
            ...
            if let Some(function_def) = self.used_untyped_function_defs.get(&fake_name) {
                ...
                self.new_function_defs.values().flat_map(|it| it.iter()).find(|it| name == it.name)
            }
        }
    }
...
impl <'a> TypeConversionContextIterator<'a> {
    ...
    fn new(context: &'a TypeConversionContext) -> Self {
        Self { index: 0, used: true, context }
    }
...
impl <'a> Iterator for TypeConversionContextIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.used {
            if let Some((_,found)) = self.context.used_untyped_function_defs.iter().nth(self.index) {
                self.index += 1;
                Some(found.clone())
            } else {
                self.used = false;
            ...
            let result = self.context.new_function_defs.values().flat_map(|it| it.iter()).nth(self.index).cloned();
            self.index += 1;
            result
        }
...
mod tests {
    ...
    fn test() {
        ...
        assert!(context.try_add_new(&"f".into(), &simple_function_def("newFun")).is_none());
        assert!(context.try_add_new(&"ff".into(), &simple_function_def("anotherNewFun")).is_none());

        assert!(context.get("aFun").is_some());
        assert!(context.get("newFun").is_some());
        assert!(context.get("anotherNewFun").is_some());

        assert_eq!(context.iter().map(|it| it.name).collect::<Vec<String>>(), vec!["aFun", "newFun", "anotherNewFun"]);
    }

    fn simple_function_def(name: &str) -> ASTFunctionDef {
        ASTFunctionDef{ name: name.into(), body: ASTFunctionBody::ASMBody("".into()), parameters: Vec::new(), return_type: None, inline: false, param_types: Vec::new()}
    }
