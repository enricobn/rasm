...
Generic types in generic functions cannot be constrained; they can be any type. The compiler will determine the types and the correctness of the function, depending on how it is called. In the above example, if we call it with an int, it will return an int, if is defined a function "add" that takes an int and returns an int, but if there is no such function, we'll get a compilation error.
...
They can be useful to disambiguate the function to call when we have a generic function, for example:

```rasm
pub fn sum<M,T>(v: M<T>) -> T {
    v.foldLeft(T::zero(), fn(accum, current) {
        accum.add(current)
    })
}
```

but for now it is not mandatory, the compiler should be able to determine the right function even without the associated function.  

In the future it could became mandatory to call such a function specifying the type, since without the type, the above functions have the same signature, and that will be not permitted.

Actually even the enum variant contructors, that are automatically generated by the compiler, are associated functions, and we can call them accordingly, for example:  
